...
def generate_roc(testing_output_normal, num_per_normal_cluster, testing_output_abnormal, num_per_abnormal_cluster, roc_point_filename):
    ...
    # base_thresholds = [1e-4, 1e-3, 1e-2]
    base_thresholds = [0.0038913958985908434, 0.003986418313670039]
    # thresholds = [elem * (i/64.0) for elem in base_thresholds for i in range(64,640)]
    thresholds = np.linspace(base_thresholds[0], base_thresholds[1], 5000)
    print(thresholds)
    roc_points = []
    for threshold in thresholds:
        ...
        roc_points.append((false_positives/num_per_normal_cluster, true_positives/num_per_abnormal_cluster, threshold))
...
generate_roc(testing_output_normal, 200, testing_output_abnormal, 213, roc_points_filename)
